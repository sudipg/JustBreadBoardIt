...
class Visual_rep(object):
	...
	def insert(self, label, type_of, length, height):
		...
		self.canvas.create_text((length*12)/2 + self.minimum_x, 40, anchor=W, font="Purisa", text="(" + type_of + ")")

		h_countdown = height
		
		local_top_y = random.randint(6, 20)*10
		local_bottom_y = local_top_y + 10
		...
		while (h_countdown > 0):
			...
			while (l_countdown > 0):
				...
				self.canvas.create_text((local_top_x + local_bottom_x)/2, (local_top_y + local_bottom_y)/2, anchor=W, font="Purisa 9", text=str(pin_number), )
	...
	def connect(self, component_1, pin_1, component_2, pin_2):
		...
		id = self.canvas.create_line(x1, y1, x2, y2, fill="red", width=2)
